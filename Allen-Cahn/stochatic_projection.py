...
class gradientfree(nn.Module):
    ...
    def loss_PDE(self,x_to_train_f,ut_minus1,ut,n_index,inv_mat,scheme='crank-nicolson'):
        """
        ...
        if scheme == 'Euler':
            f = ut - ut_minus1 - dt*(nu*(u_xx+u_yy) + ut_minus1 - ut_minus1**3)
            
        elif scheme == 'crank-nicolson':
            u_x_y_m1 = self.grad1(x_to_train_f,ut_minus1,n_index,inv_mat)                                        
            u_xx_yy_m1 = self.grad2(x_to_train_f,u_x_y_m1,n_index,inv_mat)                                                                                                              
            # u_x = u_x_y_m1[:,:,[0]]
            # u_y = u_x_y_m1[:,:,[1]]
          
            u_xx_m1 = u_xx_yy_m1[:,:,[0]]
            u_yy_m1 = u_xx_yy_m1[:,:,[3]]
            
            ft_minus1 = nu*(u_xx_m1 + u_yy_m1) + ut_minus1 - ut_minus1**3
            ft_t = nu*(u_xx + u_yy) + ut - ut**3
                                     
            f = ut - ut_minus1 - 0.5*dt*( ft_minus1 + ft_t )
        else:
            raise Exception("Scheme does not exist, supports 'Euler' and 'crank-nicolson'")
            
        f_hat =  torch.zeros(f.shape).to(x_to_train_f.device)
        loss_f = self.loss_function(f,f_hat)
        return  loss_f

    def loss(self,y_pred,y_dash,y_minus1,lambda_bc=1,lambda_pde=1):
        """
        ...
        up = torch.hstack([tp_u, bt_u, lt_u, rt_u])
        usol = torch.hstack([tp_usol, bt_usol, lt_usol, rt_usol])
        
        # Physics including the boundary
        ut = y_pred.reshape(batch_size, N_f*N_f, 1)
        ut_m1 = y_minus1.reshape(batch_size, N_f*N_f, 1)
        
        loss_u = self.loss_BC(up,usol)
        loss_f = self.loss_PDE(self.x_f_train.to(device), ut_m1, ut, 
