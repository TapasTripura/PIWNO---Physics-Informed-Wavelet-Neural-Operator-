...
x = reader.read_field('x')[:,0]#[None,None,:]   
y = reader.read_field('x')[:,0]#[None,None,:]   
...
# data = F.interpolate(data, size=[128,128], mode='bicubic', align_corners=True) 
# x = F.interpolate(x, size=[128], mode='linear', align_corners=True)[0,0,:]
# y = F.interpolate(y, size=[128], mode='linear', align_corners=True)[0,0,:]
...
T = 10
step = 1
in_channel = T_in+2               # a(x, y), x, y for this case
...
gf = stp.gradientfree(lb, ub, N_f=h, nr=9, radius=0.0315)      # Stochastic-Gradient free derivative
...
for ep in range(epochs):
    ...
    for xx, yy in train_loader:
        ...
        for t in range(0, T, step):
            y = yy[:, t:t + step, :]
            im = model(xx)
            physics_loss += gf.loss(y_pred=im.squeeze(1), y_dash=y.squeeze(1),
                                    y_minus1=xx[:,-1,:], lambda_bc=500)
            if t == 0: 
                pred = im
            else:
    ...
    t2 = default_timer()
    print('Epoch %d - Time %0.4f - Train (MSE) %0.4f - Test (L2 relative) %0.4f' 
          % (ep, t2-t1, train_pl, test_l2_step))
...
with torch.no_grad():
    ...
    for xx, yy in test_loader:
        ...
        print("Batch-{}, Test-loss-step-{:0.6f}, Test-loss-batch-{:0.6f}".format(
            index, test_l2_step/ntest/(T/step), test_l2_batch/ntest) )
...
for tvalue in range(T):
    if tvalue % 2 == 0:
        ...
        if index == 0:
            ...
            ax[2,index].set_ylabel('Absolute Error'); 
        
        index = index + 1

# %%
""" For saving the trained model and prediction data """
torch.save(model, 'model/ns_wno_allencan')
scipy.io.savemat('pred/ns_wno_allencan.mat', mdict={'prediction': prediction.cpu().numpy()})
