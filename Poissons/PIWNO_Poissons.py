"""
This code is for 2D Poisson's equation,
    - The problem is a time-independent problem.
    - 2D WNO is used here.
"""
...
# %%
""" Load the data """
TRAIN_PATH = directory + '/Data/u_sol_poissons.mat'
reader = scipy.io.loadmat(TRAIN_PATH)  	                # Load data from file
x = torch.tensor(reader['x1d'][0], dtype = torch.float)                                   # 200 points between -1 and 1 [256x1]
y = torch.tensor(reader['y1d'][0], dtype = torch.float)                                   # 200 time points between 0 and 1 [100x1] 
x_data = torch.tensor(reader['mat_sd'],dtype = torch.float) 
y_data = torch.tensor(reader['sol'],dtype = torch.float) 
...
xgrid, ygrid = xgrid.tile(ntrain+ntest,1,1), ygrid.tile(ntrain+ntest,1,1)
...
x_train = torch.stack(( x_train, xgrid[:ntrain], ygrid[:ntrain] ), dim=1) # Shape: Batch * Channel * x * y
x_test = torch.stack(( x_test, xgrid[:ntest], ygrid[:ntest] ), dim=1)     # Shape: Batch * Channel * x * y
...
lb = np.array([-1, -1])           # Lower-bound of the grid
ub = np.array([1, 1])           # Upper-bound of the grid

gf = stp.gradientfree(lb, ub, N_f=h, nr=12, radius=0.0625)      # Stochastic-Gradient free derivative

# %%
""" The model definition """
model = WNO2d(width=width, level=level, layers=layers, size=[h,s], wavelet=wavelet,
              in_channel=in_channel, padding=1).to(device)
...
for ep in range(epochs):
    ...
    for x, y in train_loader:
        ...
        x = x_normalizer.decode(x[:,0,:,:])
        
        physics_loss = gf.loss(a_xy=x,y_pred=out,y_dash=y,lambda_bc=20)
...
torch.save(model, 'model/model_phy_cwno_poissons')
scipy.io.savemat('pred/prediction_pinn_wno_poissons.mat', mdict={'pred': pred.cpu().numpy()})
