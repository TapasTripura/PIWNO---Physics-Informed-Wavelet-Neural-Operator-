...
class gradientfree(nn.Module):
    ...
    def loss_PDE(self,a_xy,x_to_train_f,u,n_index,inv_mat):
        """
    ...
    def loss(self,a_xy,y_pred,y_dash,lambda_bc=1,lambda_pde=1):
        """
        ...
        a = a_xy.reshape(batch_size, N_f*N_f, 1)
        
        loss_u = self.loss_BC(up,usol)
        loss_f = self.loss_PDE(a, self.x_f_train.to(device), u,
